      ...
      <div className="mx-auto max-w-6xl">
        ...
        <div className="grid gap-6 sm:grid-cols-2">
          {settings.qrItems.map((bank, index) => (
            <article key={`${bank.accountNumber}-${index}`} className="relative overflow-hidden rounded-[2rem] border border-white/70 bg-white/72 p-5 text-center shadow-card backdrop-blur sm:p-6">
              <div className="absolute inset-x-5 top-0 h-px bg-gradient-to-r from-transparent via-dune/35 to-transparent" />
              <div className="relative mx-auto grid aspect-square w-full max-w-64 place-items-center rounded-[2rem] border border-dune/20 bg-porcelain p-5 shadow-soft">
                <WeddingImage src={isLoading ? null : bank.qrImage} alt={`QR ${bank.ownerName}`} fill sizes="256px" className="object-contain p-6" />
              </div>
